...
def sample_from_prior(prior_bounds: Dict[str, Tuple[float, float]], n_samples: int) -> np.ndarray:
    """
...
def compute_kernel_bandwidth(population: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    weighted_var = np.average((population - weighted_mean) ** 2, axis=0, weights=weights)
...
def run_abc(
    ...
    for gen in range(1, n_populations):
        ...
        while len(new_population) < population_size and attempts < max_attempts:
            ...
            if dist < epsilon and np.isfinite(dist):
                ...
                for j in range(len(prev_population)):
                    kernel_prob = np.prod(
                        np.exp(-0.5 * ((perturbed - prev_population[j]) / bandwidth) ** 2) /
        ...
        if len(new_population) < population_size // 2:
            if verbose:
                print(f"      Warning: Only got {len(new_population)} particles, stopping early.")
            break
